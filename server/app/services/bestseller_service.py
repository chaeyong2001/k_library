...
class BestsellerService:
    ...
    async def refresh(self, source: str | None = None) -> dict:
        ...
        for item_source in sources:
            try:
                records = await self._provider(item_source).fetch(category="종합", limit=50)
                today = datetime.now(timezone.utc).date().isoformat()
                self.db.execute(delete(BestsellerItem).where(BestsellerItem.source == item_source, BestsellerItem.category == "종합"))
                for record in records:
                    self.db.add(BestsellerItem(
                        source=record.source, source_item_id=record.source_item_id, category=record.category,
                        rank=record.rank, previous_rank=record.previous_rank, title=record.title, author=record.author,
                        publisher=record.publisher, publication_date=record.publication_date, isbn10=record.isbn10,
                        isbn13=record.isbn13, cover_url=record.cover_url, source_product_url=record.source_product_url,
                        ranking_date=today,
                    ))
                self._record_run(item_source, "success", "")
                result[item_source] = "success"
            except Exception:
                self._record_run(item_source, "failed", "베스트셀러 갱신에 실패했습니다. 기존 데이터가 유지됩니다.")
                result[item_source] = "failed"
        self.db.commit()
        return result
